...
import { useParams, useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { ShoppingCart, Check, Loader, Heart, ArrowLeft, ShieldCheck, Truck, RefreshCcw } from 'lucide-react';
import Header from '../components/homepagecomponent/AppHeaders';
import { addToCart, selectCartIsUpdating } from '../redux/slices/cartSlice';
import { selectIsAuthenticated } from '../redux/slices/authSlices';
import { showToast } from '../components/ui/Toast';
...
const ProductDetailsPage = () => {
  ...
  const handleAddToCart = async () => {
    ...
    setIsAdding(true);
    try {
      await dispatch(addToCart({ productId: parseInt(productId), quantity })).unwrap();
      setIsAdded(true);
      showToast('Added to cart successfully!', 'success');
      setTimeout(() => setIsAdded(false), 2000);
    } catch (err) {
      showToast(err || 'Failed to add', 'error');
    } finally {
      setIsAdding(false);
    }
  };
  ...
  if (error || !product) return (
    <div style={{ padding: '100px 20px', textAlign: 'center' }}>
      <h2 style={{ fontSize: '2rem', marginBottom: '20px' }}>Oops! Product missing.</h2>
      <button onClick={() => navigate('/dashboard')} style={{ background: '#2563eb', color: '#fff', padding: '12px 24px', borderRadius: '8px', border: 'none', cursor: 'pointer' }}>Return to Shop</button>
    </div>
  );
  ...
  return (
    <div style={{ background: '#ffffff', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <main style={{ maxWidth: '1300px', margin: '0 auto', padding: '40px 20px' }}>
        ...
        <button 
          onClick={() => navigate(-1)} 
          style={{ display: 'flex', alignItems: 'center', gap: '8px', border: 'none', background: 'none', color: '#64748b', cursor: 'pointer', marginBottom: '32px', fontWeight: '500' }}
        >
          <ArrowLeft size={18} /> Back to Collection
        </button>

        <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth > 992 ? '1.2fr 1fr' : '1fr', gap: '60px', alignItems: 'start' }}>
          
          {/* LEFT COLUMN: VISUALS */}
          <section>
            <div style={{ position: 'relative', borderRadius: '24px', overflow: 'hidden', background: '#f8fafc', border: '1px solid #f1f5f9', marginBottom: '20px', height: '600px' }}>
              <img 
                src={product.images?.[selectedImageIndex]?.image_url || product.primary_image_url} 
                alt={product.name}
            ...
            <div style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '10px' }}>
              {product.images?.map((img, idx) => (
                <div 
                  ...
                  style={{ 
                    width: '80px', height: '80px', borderRadius: '12px', cursor: 'pointer', overflow: 'hidden',
                    border: selectedImageIndex === idx ? '2px solid #2563eb' : '2px solid transparent',
                    transition: '0.2s'
                  }}
                >
          ...
          <section style={{ position: 'sticky', top: '40px' }}>
            <div style={{ marginBottom: '8px' }}>
              <span style={{ color: '#2563eb', fontWeight: '600', fontSize: '14px', textTransform: 'uppercase', letterSpacing: '1px' }}>
                {product.category?.name}
              </span>
            </div>
            
            <h1 style={{ fontSize: '2.5rem', fontWeight: '800', color: '#0f172a', marginBottom: '16px', lineHeight: '1.1' }}>
              {product.name}
            </h1>

            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
              <span style={{ fontSize: '2rem', fontWeight: '700', color: '#0f172a' }}>
                ₹{(product.base_price / 100).toLocaleString('en-IN')}
              </span>
              {product.stock_quantity > 0 ? (
                <span style={{ background: '#dcfce7', color: '#166534', padding: '4px 12px', borderRadius: '99px', fontSize: '14px', fontWeight: '600' }}>
                  In Stock ({product.stock_quantity})
                </span>
              ) : (
                <span style={{ background: '#fee2e2', color: '#991b1b', padding: '4px 12px', borderRadius: '99px', fontSize: '14px', fontWeight: '600' }}>
                  Out of Stock
                </span>
              )}
            </div>

            <p style={{ color: '#475569', lineHeight: '1.6', fontSize: '1.1rem', marginBottom: '32px' }}>
              {product.description}
            </p>
            ...
            {product.specifications && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '40px' }}>
                {Object.entries(product.specifications).map(([key, value]) => (
                  <div key={key} style={{ padding: '12px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #f1f5f9' }}>
                    <div style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase', fontWeight: '600' }}>{key}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '4px' }}>
                  ...
                  <button onClick={() => setQuantity(Math.min(product.stock_quantity, quantity + 1))} style={qtyBtnStyle}>+</button>
                </div>
                ...
                <button 
                  ...
                  style={{ 
                    flex: 1, height: '56px', borderRadius: '14px', border: 'none', cursor: 'pointer',
                    background: isAdded ? '#10b981' : '#0f172a', color: '#fff',
                    fontWeight: '600', fontSize: '16px', transition: '0.3s',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px'
                  }}
                >
                  {isAdding ? <Loader className="animate-spin" size={20}/> : isAdded ? <Check size={20}/> : <ShoppingCart size={20}/>}
                  {isAdded ? 'Added to Cart' : 'Add to Cart'}
                </button>

                <button style={{ width: '56px', height: '56px', borderRadius: '14px', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {/* Trust Badges */}
            <div style={{ marginTop: '40px', paddingTop: '32px', borderTop: '1px solid #f1f5f9', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
               <div style={badgeStyle}><Truck size={20}/><span>Fast Delivery</span></div>
               <div style={badgeStyle}><ShieldCheck size={20}/><span>2 Year Warranty</span></div>
               <div style={badgeStyle}><RefreshCcw size={20}/><span>Easy Returns</span></div>
            </div>
          </section>
        </div>
      </main>
...
const badgeStyle = {
  ...
  fontSize: '12px', color: '#64748b', fontWeight: '500', textAlign: 'center'
};
